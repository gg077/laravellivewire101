...
<section class="mt-10 space-y-6">
    <div class="relative mb-5">
        <flux:heading>{{ __('messages.Delete account') }}</flux:heading>
        <flux:subheading>{{ __('messages.Delete your account and all of its resources') }}</flux:subheading>
    </div>
    ...
    <flux:modal name="confirm-user-deletion" :show="$errors->isNotEmpty()" focusable class="max-w-lg">
        <form wire:submit="deleteUser" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('messages.Are you sure you want to delete your account?') }}</flux:heading>
            ...
            <div class="flex justify-end space-x-2">
                ...
                <flux:button variant="danger" type="submit">{{ __('messages.Delete account') }}</flux:button>
            </div>
        </form>
    </flux:modal>
</section>
